...
按 `lxm-stock-plan.txt` 中的3条技术规则，对沪市主板(600/601/603)、深市主板(000/001)、中小板(002)、创业板(300)进行全市场筛选。
...
| R3 换手率 | 换手率保持活跃 | 近9日至少3天换手率 > 3% |
| R4 多头排列 | MA30>MA60>MA90 持续多头排列 | MA30>MA60>MA90，回溯7天至少3天对齐(已形成) |

- R1 为**必要条件**，不通过则不进入后续筛选
- R3/R4 为**优先级排序**，通过R1后计算匹配数(1-3)并降序排列
...
- R1 为必要条件，通过R1的股票全部记录并按 R3/R4 匹配数降序排列
- 实时打印所有通过R1的股票
...
with open('reports/screen_results.json') as f:
    results = json.load(f)
full = [r for r in results if int(r['matched']) >= 3]
two = [r for r in results if int(r['matched']) == 2]
one = [r for r in results if int(r['matched']) == 1]
print(f'3/3: {len(full)} | 2/3: {len(two)} | 1/3: {len(one)}')
"
```

### Step 3: 优先关注高匹配股票

所有结果已通过R1必要条件，按匹配数降序排列（3/3 > 2/3 > 1/3）。生成格式化报告：
...
- **3/3 全匹配**：蓄势+活跃换手+多头排列，最强信号
- **2/3**：R1+一条附加规则，重点关注
- **1/3 (仅R1)**：仅通过蓄势形态必要条件，待附加信号确认
- R1 不通过则不出现在结果中
